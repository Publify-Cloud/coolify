...
class Init extends Command
{
    ...
    public function handle()
    {
        ...
        try {
            ...
            echo "Continuing with initialization - cleanup errors will not prevent Publify from starting\n";
        }
        try {
        ...
        try {
            $updatedTaskCount = ScheduledTaskExecution::where('status', 'running')->update([
                'status' => 'failed',
                'message' => 'Marked as failed during Publify startup - job was interrupted',
    ...
    private function restoreCoolifyDbBackup()
    {
        if (version_compare('4.0.0-beta.179', config('constants.coolify.version'), '<=')) {
            try {
                ...
                echo "Error in restoring publify db backup: {$e->getMessage()}\n";
            }
        }
    }
