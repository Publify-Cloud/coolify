...
class ApplicationDeploymentJob implements ShouldBeEncrypted, ShouldQueue
{
    ...
    private function ensure_helper_docker_buildx_available_for_railpack(): void
    {
        ...
        throw new DeploymentException('Railpack deployments require the Docker buildx CLI plugin inside the Publify helper container. The helper could not find buildx at /root/.docker/cli-plugins/docker-buildx. Pull the latest helper image and retry the deployment.');
    }
        ...
        try {
            ...
            if ($this->newVersionIsHealthy || $force) {
                ...
                if ($this->application->dockerfile || $this->application->build_pack === 'dockerfile' || $this->application->build_pack === 'dockerimage') {
                    $this->application_deployment_queue->addLogEntry('----------------------------------------');
                    $this->application_deployment_queue->addLogEntry("WARNING: Dockerfile or Docker Image based deployment detected. The healthcheck needs a curl or wget command to check the health of the application. Please make sure that it is available in the image or turn off healthcheck on Publify's UI.");
                    $this->application_deployment_queue->addLogEntry('----------------------------------------');
                }
