# Publify Release Guide

This guide outlines the release process for Publify, intended for developers and those interested in understanding how Publify releases are managed and deployed.
...
5. **Updating the CDN**
    - To make a new version publicly available, the version information on the CDN needs to be updated manually. After that the new version number will be available at [https://cdn.publify.justahost.cloud/versions.json](https://cdn.publify.justahost.cloud/versions.json).
...
-   **Nightly**
    ...
    -   **Installation Command:**
        ```bash
        curl -fsSL https://cdn.publify.justahost.cloud/install.sh | bash -s next
        ```
...
-   **Update Availability:** New versions are available once the cloud version has been updated manually.
-   **Update Method:**
    -   Updates are managed to ensure each cloud version is thoroughly tested and stable before releasing it.

> [!IMPORTANT]
> The cloud version of Publify may be several versions behind the latest GitHub releases even if the CDN is updated. This is intentional to ensure stability and reliability for cloud users and the cloud version will be manually updated when the update is ready.
...
To update your Publify instance to a specific version, use the following command:

```bash
curl -fsSL https://cdn.publify.justahost.cloud/install.sh | bash -s <version>
```
