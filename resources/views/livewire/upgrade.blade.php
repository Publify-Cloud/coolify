        ...
        Alpine.data('upgradeModal', (config) => ({
            ...
            simulateUpgrade() {
                ...
                const steps = [
                    ...
                    { step: 3, status: '[DEV] Pulling Publify image...' },
            ...
            getReviveStatusMessage(elapsedMinutes, attempts) {
                if (elapsedMinutes === 0) {
                    return `Waiting for Publify to come back online... (attempt ${attempts})`;
                } else if (elapsedMinutes < 2) {
                    return `Waiting for Publify to come back online... (${elapsedMinutes} minute${elapsedMinutes !== 1 ? 's' : ''} elapsed)`;
                ...
                this.checkUpgradeStatusInterval = setInterval(async () => {
                    try {
                        ...
                        if (!this.serviceDown) {
                            ...
                            this.currentStep = 4;
                            this.currentStatus = 'Publify is restarting with the new version...';
                            if (this.checkUpgradeStatusInterval) {
                                clearInterval(this.checkUpgradeStatusInterval);
                                this.checkUpgradeStatusInterval = null;
                            }
