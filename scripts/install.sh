...
## REGISTRY_URL - Custom registry URL for Docker images (default: public.ecr.aws/g6l4g2t4)
...
if [ -z "$INSTALLED_DOCKER_VERSION" ]; then
    ...
    echo " - ERROR: Docker version $INSTALLED_DOCKER_VERSION is too old. Publify requires Docker $MIN_DOCKER_VERSION or newer."
...
if [[ $- == *x* ]]; then
    bash -x /data/coolify/source/upgrade.sh "${LATEST_VERSION:-latest}" "${LATEST_HELPER_VERSION:-latest}" "${REGISTRY_URL:-public.ecr.aws/g6l4g2t4}" "true"
else
...
if [ -n "$IPV4_PUBLIC_IP" ]; then
    echo -e "You can access Publify through your Public IPV4: http://$IPV4_PUBLIC_IP:8000"
fi
if [ -n "$IPV6_PUBLIC_IP" ]; then
    echo -e "You can access Publify through your Public IPv6: http://[$IPV6_PUBLIC_IP]:8000"
fi
