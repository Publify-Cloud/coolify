...
if [[ $- == *x* ]]; then
    bash -x /data/coolify/source/upgrade.sh "${LATEST_VERSION:-latest}" "${LATEST_HELPER_VERSION:-latest}" "${REGISTRY_URL:-public.ecr.aws/g6l4g2t4}"
else
    bash /data/coolify/source/upgrade.sh "${LATEST_VERSION:-latest}" "${LATEST_HELPER_VERSION:-latest}" "${REGISTRY_URL:-public.ecr.aws/g6l4g2t4}"
fi
...
if [ -n "$IPV4_PUBLIC_IP" ]; then
    echo -e "You can access Publify through your Public IPV4: http://$(curl -4s https://ifconfig.io):8000"
fi
