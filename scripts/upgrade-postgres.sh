#!/bin/bash
## Explicit Publify internal PostgreSQL major-version migrator.
...
validate_common_requirements() {
    ...
    [ -f "$ENV_FILE" ] || fail "Missing ${ENV_FILE}. Run this on a self-hosted Publify server."
...
rollback_postgres() {
    ...
    log "Starting Publify stack with rollback database volume."
    start_stack "$CURRENT_COOLIFY_IMAGE_TAG" >>"$LOGFILE" 2>&1 || fail "Could not start Publify stack after rollback. See ${LOGFILE}."
...
upgrade_postgres() {
    ...
    log "Starting Publify internal PostgreSQL major upgrade."
    ...
    if ! docker ps -a --format '{{.Names}}' | grep -qx 'coolify-db'; then
        fail "Container 'coolify-db' was not found. Start Publify before running this script."
    fi
    ...
    log "Stopping Publify application container to prevent writes during dump."
    docker stop coolify >>"$LOGFILE" 2>&1 || true
    ...
    log "Smoke-checking restored Publify database."
    ...
    log "Starting Publify stack with PostgreSQL ${TARGET_MAJOR}."
    start_stack "$CURRENT_COOLIFY_IMAGE_TAG" >>"$LOGFILE" 2>&1 || fail "Could not start Publify stack with upgraded PostgreSQL. See ${LOGFILE}."

    log "Publify internal PostgreSQL upgrade completed successfully."
    print_rollback_instructions
}
